...
$emailData=getUserEmailData($userID);
				...
				<tbody class="allAppointment">
                  ...
                  //  check($emailData);
                      ...
                      foreach ($emailData as $key => $value) 
                      {
                        ...
                        // Assuming the provided array is named $dataArray
                        $remainderDateStr = $value['remainder_date'];

                        // Create a DateTime object from the remainder_date string
                        $remainderDateTime = DateTime::createFromFormat('d-m-Y h:i A', $remainderDateStr);

                        // Extract the date part from the DateTime object
                        $remainderDate = $remainderDateTime->format('Y-m-d');


                        $run=$con->prepare('SELECT * FROM `appointment` WHERE user_id=? AND id=? ORDER BY id DESC');
                        ...
                        if($run->execute())
                        {
                            if($run->rowCount()>0)
                            {
                                $appointmentData=$run->fetch(PDO::FETCH_ASSOC);
                            }
                            else
                            {
                                continue;
                        ...
                        if($appointmentData['remainder']=='custom')
                        {

                            $remainder=$value['remainder_date'];
                            //$remainder=date("d-m-Y", strtotime($value['r_custom_date']))." ".$value['r_custom_time'];
                        }
                        else
                        {
                            $remainder=ucwords($appointmentData['remainder'])." Before";
                        }


                        if($value['send_status']=='complete')
                        {
                            $Remainderstatus='<span class="badge" style="background:#3067EF; font-size:9px">Complete</span>';
                        }
                        else 
                        {
                            $Remainderstatus=' <span class="badge" style="background:#6fd96f; font-size:10px" appointmentID="'.$value['a_id'].'">Pending</span></a>';
                        }
                        ...
                        // Create DateTime objects for the date parts
                        $eventDate = new DateTime($remainderDate);
                        $toTime = new DateTime($appointmentData['to_time']);

                        // Set the time from $toTime to $eventDate
                        $eventDate->setTime($toTime->format('H'), $toTime->format('i'));

                        
                        // Compare the event_date and to_time with the current date and time
                        if ($eventDate > $currentDateTime) {
                            $EventStatus = '<span class="badge" style="background:#6fd96f; font-size:10px">Pending</span></a>';
                        } else {
                            $EventStatus = '<span class="badge" style="background:#3067EF; font-size:9px">Complete</span>';
                        }
                       ...
                       /* if($appointmentData['status']=='complete')
                        {
                            $EventStatus='<span class="badge" style="background:#3067EF; font-size:9px">Complete</span>';
                        }
                        else 
                        {
                         ...
                         echo '<tr>
                            <td>'.$i.'</td>
                            <td>'.ucwords($appointmentData['event_name']).'</td>
                            <td>'.ucwords($appointmentData['venue']).'</td>
                            <td>'.ucwords($appointmentData['recurrence']).'</td>
                           <td>'.ucwords($value['remainder_date']).'</td>
                           <td>'.$EventStatus.'</td>
                            <td>'.$Remainderstatus.'</td>
                        </tr>';
